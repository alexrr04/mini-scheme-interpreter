...
The interpreter's code is organized in the following components:

1. **Class Definition**:

   - `SchemeVisitor`: Main class that inherits from ANTLR's generated visitor.
   - Initializes built-in functions (`map`, `filter`) and memory management
   - Overrides visitor methods to define the behavior of the interpreter for each type of expression.

2. **Helper Functions**:

   - `parse_expression`: Converts string input to parse tree. This function is a member of the `SchemeVisitor` class.
   - `format_for_scheme`: Formats Python values to Scheme syntax
   - `run_program`: Executes Scheme programs

3. **Main Program**:
   - Command-line argument parsing
   - File execution or interactive mode handling
